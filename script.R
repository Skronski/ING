...
df_vizualization <-
  temp[, !names(temp) %in% c('quarter_1', 'quarter_2', 'quarter_3', 'month_x', 'month_y')]
...
tmp <-
  preProcess(temp[!names(temp) %in% c('ID', 'obs_date', 'default', 'month_x', 'month_y')], "range")
temp$ID <- NULL
temp$obs_date <- NULL
...
for (i in 1:length(scaled_data_2[1, ])) {
  scaled_data_2[, i] <-
    ifelse(scaled_data_2[, i] < 0.01, 0.01, scaled_data_2[, i])
}
...
my_model <-
  glm(
    formula = default ~ Var_01 + Var_02 + Var_03 + Var_04 + Var_05 + Var_06 +
...
smote_oversampled$default <-
  ifelse(smote_oversampled$default == "rare", 1, 0)
...
smote_oversampled <-
  read.csv(paste0(path, '\\', 'oversampled_df_smote_01', '.csv'))
...
test_pred <-
  predict(model_vif, test[!names(test) %in% c('default')], type = 'response')
confmatrix <-
  confusionMatrix(data = as.factor(as.numeric(test_pred > 0.5)),
...
validation_pred <-
  predict(model_vif, validation[!names(validation) %in% c('default')], type = 'response')

confmatrix_valid <-
  confusionMatrix(data = as.factor(as.numeric(validation_pred > treshold)),
                  reference = as.factor(validation$default))
confmatrix_valid
...
confmatrix_valid <-
  confusionMatrix(data = validation_pred, reference = as.factor(validation$default))
confmatrix_valid
validation_pred <- predict(fit.tree, validation, type = "prob")
...
cv_model <-
  cv.glmnet(train_matrix, y, alpha = 1, lambda = lambdas_to_try)
...
test_pred_lasso <-
  predict(model_lasso,
          s = best_lambda,
          newx = as.matrix(test[, !names(test) %in% c('default')]),
          type = 'response')

auc(test$default, test_pred_lasso)


validation_pred_lasso <-
  predict(
    model_lasso,
    s = best_lambda,
    newx = as.matrix(validation[, !names(validation) %in% c('default')]),
    type = 'response'
  )
...
roc_score_lasso_validation <-
  roc(validation$default, validation_pred_lasso)
...
confmatrix_valid_lasso <-
  confusionMatrix(data = as.factor(as.numeric(validation_pred_lasso > treshold[, 1])),
                  reference = as.factor(validation$default))
confmatrix_valid_lasso
...
validation_pred_vif <-
  predict(model_vif, validation[!names(validation) %in% c('default')], type = 'response')
roc_score_logit_vif_validation <-
  roc(validation$default, validation_pred_vif)

validation_pred_tree <- predict(fit.tree, validation, type = "prob")

roc_score_tree_validation  <-
  roc(validation$default, validation_pred_tree[, 1])
roc_score_lasso_validation <-
  roc(validation$default, validation_pred_lasso[, 1])
